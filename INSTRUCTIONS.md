# Cataloging Instructions: Assigning LC Subject Headings

When asked to suggest subject headings from a bibliographic description, use the lc-vocabularies-mcp-server tools exclusively for all lookups. All authority lookups — including LCGFT genre/form terms — MUST use the tools. Do not draw on training knowledge to supply, confirm, or supplement any heading or term. If a concept cannot be confirmed through the tools, note it as unconfirmed rather than falling back on outside knowledge.
...
- LCGFT terms are confirmed through the tools and do not require an outside-knowledge disclosure
...
- **Staying within the source:** Subject analysis must be grounded solely in the bibliographic description provided by the cataloger and in tool results. Do not use training knowledge to infer topics, supply heading forms, or fill gaps in a thin description. If the source does not support a heading, do not assign it — ask the cataloger for clarification instead.
...
- **Genre/form terms:** Always confirm LCGFT terms through `lcvocab:search_lcgft` or `lcvocab:search_lcgft_keyword` — never supply them from training knowledge.
